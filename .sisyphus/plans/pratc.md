...
- [ ] 19. CLI `serve` Command — HTTP API Mode
  ...
  **What to do**:
  - Implement HTTP API server in Go (stdlib `net/http` + lightweight router):
    ...
    - `GET /api/repos/:owner/:repo/plans?target=N&top=5` → merge plans
...
- [ ] 24. Merge Plan View — Recommended Merge Set + Ordering
  ...
  **What to do**:
  - Build the Plan page at `web/src/app/plan/page.tsx`:
    ...
    - **Plan configuration panel** (left sidebar):
      ...
      - Constraint toggles: require passing CI, exclude stale > 90d, exclude drafts
  ...
  - Fetch from `/api/repos/:owner/:repo/plans` endpoint with query params (target, mode, constraints)
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Config panel lets user set target count, formula mode, constraints
