...
func NewRateLimitGuard(budget *ratelimit.BudgetManager, metrics *ratelimit.Metrics, store *cache.Store) RateLimitGuard {
...
func (g RateLimitGuard) CheckBudget(repo string, estimatedRequests int) (int, error) {
	...
	if err := g.store.PauseSyncJob(repo, nextScheduledAt, "rate limit budget exhausted"); err != nil {
